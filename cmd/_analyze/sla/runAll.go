...
func RunAll(inFileName string) {
	...
	// Open files
	if exchOutFile, err = os.Create("files/exchanges-from-"+inFile+".dat"); err != nil {log.Fatal(err)}
	defer exchOutFile.Close()
	if lTOutFile, err = os.Create("lifetimes.dat"); err != nil {log.Fatal(err)}
	defer lTOutFile.Close()
	
	if wasInDBHFile, err = os.Create("files/wasInDBH-from-"+inFile+".dat"); err != nil {log.Fatal(err)}
	defer wasInDBHFile.Close()
	
	if lastInDBHFile, err = os.Create("files/lastInDBH-from-"+inFile+".dat"); err != nil {log.Fatal(err)}

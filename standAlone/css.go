...
func main () {
	...
	var (
		...
		baseName string
		walltime string
		kiraFileString string
		kiraString string
		pbsString string
		regString  string         = `(\w{3})-(\S*)\.txt`
	...
	regResult = regExp.FindStringSubmatch(icsName)
	if regResult[0] != "ics" {
		log.Fatal("Please specify an ICs file!!")
	}
	fileNameBody = regResult[1]
	...
	pbsString = "#!/bin/bash\n" +
		...
		modules +
		"sh " + filepath.Join(currentDir, kiraFileString) + "\n"
